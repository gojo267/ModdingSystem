...
| **IsCodeLoad** | Load mod file code into a scene variable for later use. |
...
| **IsCode** | Returns true if the provided mod file code string is valid (syntax check). |
| **IsDownloadDone** | Returns true when a mod download has completed. |
| **ErrorOrSignal** | Returns true when a modding error or signal has occurred. |
